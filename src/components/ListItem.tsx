...
function getListItemWrapperStyles(selected: boolean, disabled?: boolean) {
  return classNames(
    "command-palette-list-item block w-full text-left px-3.5 py-2.5 rounded-md hover:bg-gray-800 focus:ring-1 focus:ring-gray-300 focus:outline-none flex items-center space-x-2.5 justify-between",
    selected && !disabled
      ? "bg-gray-800"
      : "bg-transparent",
    disabled
    ...
    <>
      <div className="flex w-full items-center space-x-2.5">
        ...
        {typeof children === "string" ? (
          <span className="truncate max-w-md text-white">{children}</span>
        ) : (
          children
        )}
      </div>
